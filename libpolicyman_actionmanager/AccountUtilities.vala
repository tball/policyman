...
namespace PolicyMan.Common {
	public class AccountUtilities : GLib.Object {
		public static Gee.List<Account> get_all_accounts() {
			var all_accounts = new ArrayList<Account>();
		...
		public static Gee.List<Account> get_groups() {
			var groups = new ArrayList<Account>();
			...
			if (oobs_groups.get_iter_first(ref iter)) {
				do {
					...
					if (oobs_group.name != null && oobs_group.name != "") {
						groups.add(new Account() { user_name = oobs_group.name, full_name = oobs_group.name, account_type = AccountType.LINUX_GROUP });
					}
		...
		public static Gee.List<Account> get_users() {
			var users = new ArrayList<Account>();
			...
			if (oobs_users.get_iter_first(ref user_iter)) {
				do {
					...
					if (oobs_user.name != null && oobs_user.name != "") {
						users.add(new Account() { user_name = oobs_user.name, full_name = oobs_user.full_name, account_type = AccountType.LINUX_USER });
					}
